function out = s1_gd2D(img,SCALES)
  global DEBUG;
  %-----------------------------------
  %-----------------------------------
  [ht,wt]= size(img)
  if(nargin~=2 || isempty(SCALES))
    SCALES = logspace(log10(0.125),log10(1),12)*20
  end;
  if(isrgb(img))
    img = rgb2gray(img);
  end;
  ...
  for s = 1:length(SCALES)
      ...
      tmps1(:,:,23) = pos.^(2/3); 1./(1+exp(-pos));tmps1(:,:,24)=neg.^(2/3);%1./(1+exp(-neg));
      %----------------------------------
      %gaussian curvature
      %---------------------------------- 
	...
	if(0)
	    ...
	    tmps1(:,:,30)  = gd{1,1};
	    dx             = ceil(SCALES(s)/2);
    	out{scale}     = tmps1(1:dx:end,1:dx:end,[1:22]); %imresize(tmps1,[orght orgwt],'bilinear');
